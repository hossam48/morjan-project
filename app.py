...
def calculate_health_status(tasks):
    today = date.today()
    # تحويل العمود لتاريخ للمقارنة
    tasks['Due Date'] = pd.to_datetime(tasks['Due Date']).dt.date
    overdue_count = len(tasks[(tasks['Due Date'] < today) & (tasks['Progress'] < 100)])
...
# --- 5. واجهة المشروع التفصيلية ---
    ...
    # حسابات الصحة
    current_tasks = st.session_state.tasks
    health_status, health_icon = calculate_health_status(current_tasks.copy())
    overdue_tasks = len(current_tasks[(pd.to_datetime(current_tasks['Due Date']).dt.date < date.today()) & (current_tasks['Progress'] < 100)])
    ...
    with st.sidebar:
        ...
        # عرض حالة المشروع والمهام المتأخرة
        st.markdown("---")
        st.subheader("🏥 Health Check")
        st.metric("Project Health", f"{health_status} {health_icon}")
        if overdue_tasks > 0:
            st.error(f"⚠️ {overdue_tasks} Overdue Task(s)!")
        else:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📅 Calendar", "📊 Analytics", "📋 MDL (Editable)", "👥 Team"])
    ...
    with tab2:
        ...
        # ترتيب المهام حسب التاريخ لضمان عرض أفضل
        df_sorted = df.sort_values(by='Due Date')
        df_sorted['Start'] = pd.to_datetime(date.today())
        fig_gantt = px.timeline(df_sorted, x_start='Start', x_end='Due Date', y='Item', color='Owner', title="Timeline View (Sorted by Date)")
        fig_gantt.update_yaxes(autorange="reversed")
        st.plotly_chart(fig_gantt, use_container_width=True)

    with tab3:
        st.subheader("📋 Master Document List (Live Editor)")
        st.info("💡 Tip: You can edit any cell directly in the table below. Changes are saved automatically.")
        
        # تحويل الجدول إلى Data Editor للتعديل المباشر
        edited_df = st.data_editor(
            ...
            column_config={
                "Progress": st.column_config.ProgressColumn(
                    "Progress",
                    help="Task progress from 0% to 100%",
                    format="%d%%",
                    min_value=0,
                    max_value=100,
                ),
                ...
                "Status": st.column_config.SelectboxColumn(
                    "Status",
                    options=["Pending", "In Progress", "Technical Evaluation", "Submitted", "Completed"]
                ),
        ...
        # تحديث البيانات في الـ Session State
    ...
    with tab4:
        ...
        for member in st.session_state.team_members:
            ...
            with c_b:
                st.caption(f"{task_count} Tasks | {avg_p:.1f}% Avg")

st.markdown("<br><hr><center>Elsewedy Electric Engineering Control Suite v4.7 © 2026</center>", unsafe_allow_html=True)
