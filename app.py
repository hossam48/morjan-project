...
if st.session_state.current_project is None:
    ...
    # حساب الميزانية الإجمالية بشكل آمن
    total_budget = 0
    for p in st.session_state.projects.values():
        budget_str = p['budget']
        try:
            # استخراج الرقم من النص
            if 'SAR' in budget_str:
                num = int(budget_str.replace('SAR', '').replace('M', '').strip())
            else:
                num = int(budget_str.replace('USD', '').replace('
...
""", unsafe_allow_html=True), '').replace('M', '').strip())
            total_budget += num
        except:
            pass
